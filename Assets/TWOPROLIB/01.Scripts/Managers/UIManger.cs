...
namespace TWOPROLIB.Scripts.Managers
{
    ...
    public class UIManger : MonoBehaviour
    {
        ...
        {
            /// <summary>
            /// 게임 상태(GameState, PlayState 두가지가 가능)
            /// </summary>
            [Tooltip("게임 상태(GameState, PlayState 두가지가 가능)")]
        ...
        [Serializable]
        public class PlayUIView
        {
            /// <summary>
            /// 게임 상태(GameState, PlayState 두가지가 가능)
            /// </summary>
            [Tooltip("게임 상태(GameState, PlayState 두가지가 가능)")]
            public PlayStateType state;
            /// <summary>
            /// 해당 게임에 보여야 할 오브젝트 리스트
            /// </summary>
            [Tooltip("해당 게임에 보여야 할 오브젝트 리스트")]
        ...
        /// <summary>
        /// GameState에 따른 View List
        /// </summary>
        [SerializeField][Tooltip("GameState에 따른 View List")]
        public List<GameUIView> lsGameStatView;

        /// <summary>
        /// PlayState에 따른 View List
        /// </summary>
        [SerializeField][Tooltip("GameState에 따른 View List")]
        public List<PlayUIView> lsPlayStatView;
        ...
        public virtual void OnGameStateEvent()
        {
            ...
            for (int i = 0; i < this.lsGameStatView.Count; i++)
            {
                ...
                {
                    this.lsGameStatView[i].gameStateObj[j].SetActive(GameManager.Instance.gameState.gameState == this.lsGameStatView[i].state ? true : false);
        ...
        {
            // 해당 UI를 View하기 위한 로직
            for (int i = 0; i < this.lsPlayStatView.Count; i++)
            {
                for (int j = 0; j < this.lsPlayStatView[i].gameStateObj.Count; j++)
                {
                    this.lsPlayStatView[i].gameStateObj[j].SetActive(GameManager.Instance.playState.playState == this.lsPlayStatView[i].state ? true : false);
        ...
        public void OnTutorialStartButton()
        {
            ...
            OnChangeScene((int)ScreenManager.ScenePage.GAMEPLAY);
        }
