...
namespace TWOPRO.Scripts.Spawners
{
    public class SpawnerMaster_Laser : SpawnManager
    {
        ...
        public override void OnPlayStateEvnet()
        {
            ...
            switch(GameManager.Instance.playState.playState)
            {
                case PlayStateType.Play:
                    ...
                    for(int i = 0; i < OtherSapwner_Laser.Count; i++)
                    {
                        if (!OtherSapwner_Laser[i].gameObject.activeSelf)
                            OtherSapwner_Laser[i].gameObject.SetActive(true);

                        ParticleSystem[] p = OtherSapwner_Laser[i].GetComponentsInChildren<ParticleSystem>();

                        StartCoroutine(Laset_Start(p));
                    }
        ...
        IEnumerator Laset_Start(ParticleSystem[] p)
        {
